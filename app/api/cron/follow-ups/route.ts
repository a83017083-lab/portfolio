...
export async function GET(req:Request){
 ...
 try{const transporter=nodemailer.createTransport({host:"smtp.gmail.com",port:465,secure:true,auth:{user:process.env.GMAIL_USER,pass:process.env.GMAIL_APP_PASSWORD}});await transporter.sendMail({from:`"Portfolio Site" <${process.env.GMAIL_USER}>`,to:process.env.GMAIL_USER,subject:`Portfolio follow-ups due - ${today}`,text});const marked=await kvSet(key,true);if(!marked)return NextResponse.json({ok:true,due:due.length,warning:"Email sent, but deduplication could not be saved. A later retry may send another copy."});return NextResponse.json({ok:true,due:due.length})}catch(e){console.error("follow-up reminder failed",e);return NextResponse.json({error:"Mail failed"},{status:502})}
}
