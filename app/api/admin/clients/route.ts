...
import {readClients,addClient,tokenHash} from "../../../../lib/client-portal";
import {kvJsonClientMutate} from "../../../../lib/kv";
import {audit} from "../../../../lib/audit";
const KEY="clients:v1";
const noStore={"Cache-Control":"no-store"};
export async function GET(){
 if(!isAuthed())return NextResponse.json({error:"Unauthorized"},{status:401});
 const result=await readClients();if(!result.ok)return NextResponse.json({error:"Storage unavailable"},{status:503});
 return NextResponse.json({clients:result.clients.map(({tokenHash,...c})=>c)},{headers:noStore});
}
export async function POST(req:Request){
 ...
 if(added===null)return NextResponse.json({error:"Storage unavailable"},{status:503});
 if(!added)return NextResponse.json({error:"Client limit reached"},{status:400});
 await audit("client-created",`Client ${record.id.slice(0,8)} created`);
 return NextResponse.json({client:{id:record.id,name:record.name,project:record.project,status:record.status,update:record.update},accessCode:code},{headers:noStore});
}
export async function PATCH(req:Request){
 ...
 await audit("client-updated",`Client ${b.id.slice(0,8)} status changed`);
...
export async function DELETE(req:Request){
 ...
 if(changed===null)return NextResponse.json({error:"Storage unavailable"},{status:503});
 if(!changed)return NextResponse.json({error:"Not found"},{status:404});
 await audit("client-deleted",`Client ${id.slice(0,8)} deleted`);
 return NextResponse.json({ok:true});
}
