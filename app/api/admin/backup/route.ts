...
export async function GET(){
 ...
 const [content,chatbot,inquiries,subscribers,coupons,audit,clients,reviews,integrations]=await Promise.all([
  ...
  kvReadResult<{id:string;tokenHash:string}[]>("clients:v1"),kvListResult("reviews:pending:v1",0,99),kvReadResult<{n8nWebhookUrl?:string}>("integrations:v1")
 ]);
 if(!content.ok||!chatbot.ok||!inquiries.ok||!subscribers.ok||!coupons.ok||!audit.ok||!clients.ok||(clients.value!==null&&!Array.isArray(clients.value))||!reviews.ok||!integrations.ok)return NextResponse.json({error:"Storage read failed; no incomplete backup generated"},{status:503});
 const backup={exportedAt:new Date().toISOString(),schema:"portfolio-backup-v1",note:"Export snapshot only. Restore is not implemented. Client access-code hashes are omitted; client logins must be reissued separately if storage is lost.",content:content.value||DEFAULT_CONTENT,chatbot:chatbot.value||DEFAULT_CHATBOT,inquiries:inquiries.items,subscribers:subscribers.items,coupons:coupons.value,audit:audit.items,clients:(clients.value||[]).map(({tokenHash,...c})=>c),pendingReviews:reviews.items,n8nWebhookUrl:integrations.value?.n8nWebhookUrl||""};
