...
import {kvListResult,kvReplaceList} from "../../../../lib/kv";
...
export async function DELETE(req:Request){if(!isAuthed())return NextResponse.json({error:"Unauthorized"},{status:401});const body=await req.json().catch(()=>null);if(typeof body?.id!=="string"||! /^[a-f0-9-]{36}$/.test(body.id))return NextResponse.json({error:"Invalid review ID"},{status:400});const r=await kvListResult<Review>(KEY,0,99);if(!r.ok)return NextResponse.json({error:"Storage unavailable"},{status:503});if(!r.items.some(x=>x.id===body.id))return NextResponse.json({error:"Review not found"},{status:404});if(!await kvReplaceList(KEY,r.items.filter(x=>x.id!==body.id)))return NextResponse.json({error:"Storage unavailable"},{status:503});await audit("review-delete","Pending review removed");return NextResponse.json({ok:true})}
