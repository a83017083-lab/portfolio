...
export default function HealthCheck(){
 ...
 async function check(){setChecking(true);setResults([]);setAsOf(null);const next:Result[]=[];
  for(const item of CHECKS){const start=performance.now();try{const ctrl=new AbortController();const timeout=setTimeout(()=>ctrl.abort(),8000);let res:Response;try{res=await fetch(item.path,{cache:"no-store",credentials:"same-origin",signal:ctrl.signal})}finally{clearTimeout(timeout)}next.push({path:item.path,code:res.status,duration:Math.round(performance.now()-start),ok:(item.expect as readonly number[]).includes(res.status),note:item.note})}catch{next.push({path:item.path,code:null,duration:Math.round(performance.now()-start),ok:false,note:item.note})}}
