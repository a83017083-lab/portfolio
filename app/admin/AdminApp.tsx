...
function ContentEditor({
  ...
  return (
    <>
      ...
      <div className="admin-panel"><h2>Page headings and footer</h2><p className="hint">Major public-page introduction text and footer lines.</p>{(Object.keys(content.pageCopy||{}) as (keyof SiteContent["pageCopy"])[]).map(k=><div key={k} className="content-editor-item"><b>{k}</b><label className="admin-field"><span>Heading</span><input maxLength={120} value={content.pageCopy[k].title} onChange={e=>patch(c=>({...c,pageCopy:{...c.pageCopy,[k]:{...c.pageCopy[k],title:e.target.value}}}))}/></label><label className="admin-field"><span>Introduction</span><textarea maxLength={500} value={content.pageCopy[k].description} onChange={e=>patch(c=>({...c,pageCopy:{...c.pageCopy,[k]:{...c.pageCopy[k],description:e.target.value}}}))}/></label></div>)}<label className="admin-field"><span>Footer brand line</span><input maxLength={180} value={content.footer?.brandLine||""} onChange={e=>patch(c=>({...c,footer:{...c.footer,brandLine:e.target.value}}))}/></label><label className="admin-field"><span>Footer bottom line</span><input maxLength={180} value={content.footer?.bottomLine||""} onChange={e=>patch(c=>({...c,footer:{...c.footer,bottomLine:e.target.value}}))}/></label></div>
      ...
      <div className="admin-panel">
        ...
        <div className="admin-field">
          <span>Hero headline (first line)</span><input maxLength={80} value={content.hero.headlineFirst} onChange={e=>patch(c=>({...c,hero:{...c.hero,headlineFirst:e.target.value}}))}/><span>Hero headline (accent line)</span><input maxLength={80} value={content.hero.headlineSecond} onChange={e=>patch(c=>({...c,hero:{...c.hero,headlineSecond:e.target.value}}))}/>
...
function BackupAdmin(){return <><h1>Private backup</h1><p className="page-sub">Download a JSON snapshot of site content, inquiries, subscribers, private coupons, pending reviews, referral invoice credits, the n8n webhook URL, client records without access-code hashes, and recent activity. Keep it private; it contains visitor and invoice information.</p><div className="admin-panel"><h2>Export</h2><a className="admin-btn" href="/api/admin/backup" download="portfolio-backup.json">Download backup JSON</a><p className="hint" style={{marginTop:15}}>Restore is deliberately unavailable: overwriting live inquiries could lose newer messages. Keep a copy offline.</p></div></>}
